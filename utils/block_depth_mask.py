...
class BlockDepthMasker:
    def __init__(
        ...
        print(
            ...
            f"dilate_px={self.dilate_px} cache={self.cache_masks} "
    ...
    def mask_for(self, viewpoint_cam):
        ...
        key = self._cache_key(viewpoint_cam, width, height)
        if self.cache_masks and key in self._mask_cache:
            return self._mask_cache[key], True

        mask = self._build_mask(viewpoint_cam, width, height)
        if self.cache_masks:
            self._store_mask(key, mask)
        return mask, False
    ...
    def _cache_key(self, viewpoint_cam, width, height):
        image_name = getattr(viewpoint_cam, "image_name", "")
        return (image_name, int(width), int(height))

    def _store_mask(self, key, mask):
        if self.cache_max_items > 0:
            while len(self._mask_cache) >= self.cache_max_items:
                self._mask_cache.popitem(last=False)
        self._mask_cache[key] = mask

    def _build_mask(self, viewpoint_cam, width, height):
