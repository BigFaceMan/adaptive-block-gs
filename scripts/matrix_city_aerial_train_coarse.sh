...
CONFIG="${CONFIG:-config/mc_aerial_coarse.yaml}"
CUDA_ID="${CUDA_ID:-0}"
...
emit("CFG_SOURCE_PATH", train_args["source_path"])
...
TEST_RENDER_DIR="$CFG_MODEL_PATH/test/ours_${CFG_RENDER_ITERATION}/renders"
...
render_test_set() {
    ...
    echo "Rendering coarse GS test set"
...
run_metrics() {
    ...
    echo "Evaluating coarse GS test renders"
...
if [ "$RUN_TRAIN" = "1" ]; then
    if [ "$SKIP_TRAINED" = "1" ] && [ -f "$MODEL_PLY" ]; then
        echo "Coarse model already exists: $MODEL_PLY"
...
if [ ! -f "$MODEL_PLY" ]; then
    echo "Coarse model not found: $MODEL_PLY" >&2
    exit 1
fi
