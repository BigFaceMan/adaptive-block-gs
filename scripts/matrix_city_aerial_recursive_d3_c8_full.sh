...
CONFIG="${CONFIG:-config/mc_aerial_recursive_d3_c8.yaml}"
CUDA_IDS="${CUDA_IDS:-0}"
...
partition_args=(
    --config "$CONFIG"
    --override partition.max_depth=3
    --override partition.max_blocks=8
)
...
if [ "$RUN_PARTITION" = "1" ]; then
    if [ -f "$PARTITION_TREE" ] && [ "$FORCE_PARTITION" != "1" ]; then
        ...
        echo "Running recursive partition with max depth 3 and max 8 blocks..."
