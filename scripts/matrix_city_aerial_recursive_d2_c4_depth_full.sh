...
partition_args=(
    --config "$CONFIG"
    --override partition.max_depth=2
    --override partition.max_blocks=4
)
...
if [ "$RUN_PARTITION" = "1" ]; then
    if [ -f "$PARTITION_TREE" ] && [ "$FORCE_PARTITION" != "1" ]; then
        ...
        echo "Running recursive partition with max depth 2 and max 4 blocks..."
...
echo "Running depth-reg block training / merge / render eval / metrics..."
CONFIG="$CONFIG" \
CUDA_IDS="$CUDA_IDS" \
