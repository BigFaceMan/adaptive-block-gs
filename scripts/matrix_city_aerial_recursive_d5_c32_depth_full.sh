...
CONFIG="${CONFIG:-config/mc_aerial_recursive_d5_c32_depth.yaml}"
...
partition_args=(
    --config "$CONFIG"
    --override partition.max_depth=5
    --override partition.max_blocks=32
)
...
if [ "$RUN_PARTITION" = "1" ]; then
    if [ -f "$PARTITION_TREE" ] && [ "$FORCE_PARTITION" != "1" ]; then
        ...
        echo "Running recursive partition with max depth 5 and max 32 blocks..."
        python partition.py "${partition_args[@]}"
    fi
else
    echo "RUN_PARTITION=0, reusing partition: $PARTITION_TREE"
fi
...
echo "Running d5/c32 depth-reg block training / merge / render eval / metrics..."
CONFIG="$CONFIG" \
CUDA_IDS="$CUDA_IDS" \
